import{randomUUID}from"node:crypto";import{z}from"zod";import{registerAppTool}from"@modelcontextprotocol/ext-apps/server";import type{McpServer}from"@modelcontextprotocol/sdk/server/mcp.js";import{DEFAULT_WEIGHTS,type GrantWatch,type GrantWatchFrequency,type GrantWatchMatchQuality,type GrantWatchNotificationType,type GrantWatchScope,type MatchWeights}from"../../domain/types.js";import{grantRepository}from"../../repositories/grantRepository.js";import{inferRequestedResultCount,MAX_SEARCH_RESULTS,rescoreGrants,searchGrants}from"../../services/grantSearchService.js";import{emailService}from"../../services/emailService.js";import{mergeProfilesFromRequest}from"../../services/profileInference.js";import{buildWatchConfirmationEmail}from"../../services/watchEmailTemplates.js";import{maskWatchEmail,watchOwnerKey}from"../../services/watchOwnership.js";import{nextWatchCheck,qualityFromMinimumScore,WATCH_FREQUENCY,WATCH_NOTIFICATION_LABELS,WATCH_QUALITY}from"../../services/watchPreferences.js";import{buildWatchGrantState}from"../../services/watchService.js";import{GRANTPILOT_WIDGET_URI}from"../resources/grantPilotWidget.js";
...
const publicWatch=(watch:GrantWatch)=>{const{lastSeenGrantState:_internalBaseline,ownerKey:_owner,unsubscribeToken:_unsubscribeToken,email,...value}=watch;return{...value,email:maskWatchEmail(email)}};
...
export function registerGrantTools(server:McpServer){
 ...
 server.registerTool("create_grant_watch",{description:"Create a customizable, owner-scoped GrantPilot email watch. Prefer plain-language matchQuality instead of asking users to choose a score: worth-reviewing is broad and recommended, strong is more selective, and top-only is rare and highly selective. Users can choose the watched scope, alert reasons, deadline reminder window, and delivery cadence. Azure Communication Services Email sends the confirmation when configured.",inputSchema:z.object({queryId:z.string(),email:z.string().email(),matchQuality:z.enum(["worth-reviewing","strong","top-only"]).optional().describe("Plain-language alert sensitivity. Default to worth-reviewing unless the user explicitly wants fewer, stronger matches."),minimumScore:z.number().min(0).max(100).optional().describe("Legacy compatibility only. Prefer matchQuality so users do not need to understand a numeric score."),frequency:z.enum(["as-detected","daily","weekly"]).default("daily").describe("Email delivery cadence. Daily is the recommended low-noise default."),scope:z.enum(["search","selected-grant"]).default("search").describe("Watch the full saved search or only the selected grant."),deadlineLeadDays:z.number().int().min(1).max(90).default(14),notificationTypes:z.array(z.enum(["new-match","deadline-change","opportunity-amended","opportunity-closing","opportunity-closed","opportunity-removed","no-longer-matching","score-increased"])).min(1).default(["new-match","opportunity-closing","opportunity-closed","opportunity-removed","no-longer-matching"]),selectedGrantId:z.string().optional(),copilotReturnUrl:z.string().url().optional().describe("Microsoft 365 Copilot or Teams conversation URL supplied by the host. Used only for the Open in Copilot email button.")}),_meta:{ui:{visibility:["app","model"]}}},async(input,extra)=>{
  ...
  const existing=grantRepository.findEquivalentWatch(ownerKey,{queryId:input.queryId,email:input.email,scope,selectedGrantId:scope==="selected-grant"?input.selectedGrantId:undefined});
  if(existing?.status==="active"){
   return{content:[{type:"text",text:"This watch is already active for the same search, scope, and email address."}],structuredContent:{...publicWatch(existing),duplicatePrevented:true}};
  }
  const watch:GrantWatch={
   id:existing?.id??`watch-${randomUUID().slice(0,8)}`,
   queryId:input.queryId,
   email:input.email,
   ownerKey,
   unsubscribeToken:existing?.unsubscribeToken??randomUUID().replaceAll("-",""),
   matchQuality,
   ...
   lastSeenGrantState:buildWatchGrantState(search),
  };
  await grantRepository.saveWatch(watch);
  const message=buildWatchConfirmationEmail(watch,search,selected);
  let delivery;
  try{
   delivery=await emailService.send({to:input.email,...message});
  }catch(error){
   delivery={provider:"azure-communication-services"as const,status:"failed"as const,error:error instanceof Error?error.message:"Unknown confirmation-email failure"};
  }
  const finalized:GrantWatch={
   ...watch,
   status:delivery.status==="failed"?"confirmation-failed":"active",
  };
  await grantRepository.saveWatch(finalized);
  ...
  if(delivery.status==="failed")return{isError:true,content:[{type:"text",text:"The confirmation email could not be sent, so the watch was saved as inactive instead of silently running. Retry after checking the Azure email configuration; GrantPilot will reuse this watch rather than create a duplicate."}],structuredContent:{...publicWatch(finalized),settingsSummary,error:{code:"WATCH_CONFIRMATION_FAILED",message:delivery.error},emailDelivery:{subject:message.subject,status:delivery.status,provider:delivery.provider}}};
  return{content:[{type:"text",text:`Watch created for ${WATCH_QUALITY[matchQuality].label.toLowerCase()} with a ${WATCH_FREQUENCY[frequency].label.toLowerCase()}. The confirmation email was sent.`}],structuredContent:{...publicWatch(finalized),settingsSummary,emailDelivery:{subject:message.subject,status:delivery.status,provider:delivery.provider,messageId:delivery.messageId}}};
 });
 server.registerTool("list_grant_watches",{description:"List only the GrantPilot watches owned by the current authenticated principal or MCP session. Email addresses are masked in the result.",inputSchema:z.object({}),annotations:{readOnlyHint:true}},async(_input,extra)=>{const watches=grantRepository.listWatchesForOwner(watchOwnerKey(extra));return{content:[{type:"text",text:`${watches.length} watches belong to this session.`}],structuredContent:{watches:watches.map(publicWatch)}}});
 server.registerTool("delete_grant_watch",{description:"Delete a GrantPilot watch owned by the current authenticated principal or MCP session.",inputSchema:z.object({watchId:z.string()}),annotations:{destructiveHint:true}},async({watchId},extra)=>{const ownerKey=watchOwnerKey(extra);const deleted=await grantRepository.deleteWatch(watchId,ownerKey);return{content:[{type:"text",text:deleted?"Watch deleted.":"Watch was not found for this session."}],structuredContent:{watchId,deleted}}});
