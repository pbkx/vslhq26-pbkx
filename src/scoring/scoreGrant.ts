...
export function scoreGrant(grant:GrantOpportunity,org:OrganizationProfile,project:ProjectProfile,historical:HistoricalEvidence,weights:MatchWeights=DEFAULT_WEIGHTS):GrantResult{
 ...
 const components={
  ...
  applicantEligibility:component(applicantScore,weights.applicantEligibility,privateProspect?25:grant.eligibleApplicantTypes.length?95:45,[privateProspect?"Evidence-backed potential private donor/funder candidate worth researching and possibly contacting.":explicitlyExcluded?"Applicant type conflicts with the stated eligible categories.":"Nonprofit applicant type is supported or not explicitly excluded."],[evidence("organization","organizationType",org.organizationType),evidence("opportunity","eligibleApplicantTypes",grant.eligibleApplicantTypes.join(", "))],privateProspect?["Research the funder's current outreach and relationship criteria."]:grant.eligibleApplicantTypes.length?[]:["Detailed applicant categories are missing."]),
  geographicFit:component(
   ...
   [privateProspect
    ?historicalStateMatch?"Historical IRS records include giving aligned with the project's state.":"Research the funder's current geographic priorities."
   ...
   privateProspect?["Research the funder's current geographic priorities."]:eligibleStates.length||nationwide?[]:["Eligible states were not specified."]
  ),
  ...
  deadlineFeasibility:component(deadline,weights.deadlineFeasibility,privateProspect?15:grant.deadline?95:35,[privateProspect?"Build an outreach timeline through current foundation research.":days===undefined?"Deadline was not supplied.":`${days} days remain at ${org.applicationCapacity??"medium"} application capacity.`],[evidence("opportunity","deadline",grant.deadline??"Relationship outreach"),evidence("organization","applicationCapacity",org.applicationCapacity??"medium")],grant.deadline?[]:privateProspect?["Research the foundation's current outreach calendar."]:["Deadline is unknown."])
 };
